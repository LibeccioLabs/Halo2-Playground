...
impl<const N_OBJECTS: usize, F: ff::Field> PermutationChip<N_OBJECTS, F> {
    ...
    pub fn apply_permutation(
        ...
        layouter.assign_region(
            || "load input",
            |region| {
                Self::apply_permutation_region_assignment(&self, &input_items, permutation, region)
            },
        )
    }

    /// A helper function to be used in Self::apply_permutation.
    /// Its main purpose is to increase readability by reducing indentation.
    fn apply_permutation_region_assignment<'a>(
        &self,
        input_items: &[Number<F>; N_OBJECTS],
        permutation: [usize; N_OBJECTS],
        ...
        self.config.s_perm.enable(&mut region, 0)?;
        ...
        let mut next_free_cell = DivModCounter::<N_OBJECTS>::new_const_mod(1, 0)
            .map(|(row_idx, col_idx)| (self.config.item_columns[col_idx], row_idx));

        // An iterator that visits the cells of `swap_selector_columns`, row by row.
        let mut next_swap_selector = DivModCounter::<0, true>::new_runtime_mod(
            0,
            0,
            self.config.swap_selector_columns.len(),
        )
        .map(|(row_idx, col_idx)| (self.config.swap_selector_columns[col_idx], row_idx));

        let mut item_tracker: [Number<F>; N_OBJECTS] = (0..N_OBJECTS)
            .map(|idx| input_items[idx].clone())
            .f_collect("the number of items is correct");

        for (swap_is_applied, idx1, idx2) in
